...
checks = {
    "version_0_6_2": 'APP_VERSION = "0.6.2"' in main,
    ...
    "version_file": (root / "VERSION").read_text(encoding="utf-8").strip() == "0.6.2",
    "no_bundled_runtime_db": not (root / "aion.db").exists() and not (root / ".aion-test.db").exists(),
}

external = [
    "Reconcile production v0.7.1 with recovered v0.6.2 source before merging or deploying; do not downgrade production.",
