...
def test_cloudru_gateway_root_404_retries_same_gateway_v1_before_direct(monkeypatch):
    monkeypatch.setenv("AION_REASONING_PROVIDER", "cloudru_chat_completions")
    monkeypatch.setenv("AION_REASONING_API_KEY", "gateway-secret")
    monkeypatch.setenv("AION_REASONING_BASE_URL", "https://gateway.example")
    ...
    def fake_post(url, *, headers, json, timeout):
        ...
        if url == "https://gateway.example/chat/completions":
...
def test_cloudru_temple_brain_uses_its_model_and_runtime_reports_provider(monkeypatch):
    ...
    assert status["cloudru_gateway_v1_404_fallback_enabled"] is True
